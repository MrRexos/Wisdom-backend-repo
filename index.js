...
function buildInternalRequestId(prefix, parts = []) {
  const normalizedPrefix = String(prefix || 'req').replace(/[^a-z0-9_]/gi, '').slice(0, 16) || 'req';
  const hash = crypto
    .createHash('sha256')
...
function buildRefundRequestId(parts = []) {
  return buildInternalRequestId('rr', parts);
}
...
function buildPublicPaymentMethodReference(recordId) {
  ...
  return normalizedRecordId ? `${PUBLIC_PAYMENT_METHOD_PREFIX}${normalizedRecordId}` : null;
}

function parsePublicPaymentMethodReference(value) {
  if (typeof value !== 'string') {
    return null;
  }

  const trimmedValue = value.trim();
  if (!trimmedValue.startsWith(PUBLIC_PAYMENT_METHOD_PREFIX)) {
    return null;
  }

  const rawRecordId = trimmedValue.slice(PUBLIC_PAYMENT_METHOD_PREFIX.length);
  if (!/^\d+$/.test(rawRecordId)) {
    return null;
  }

  return normalizeNullableInteger(rawRecordId);
}
...
function redactStripePaymentMethodIds(value) {
  ...
  if (Array.isArray(value)) {
    return value.map((item) => redactStripePaymentMethodIds(item));
  }
  ...
  if (value && typeof value === 'object') {
    return Object.fromEntries(
      Object.entries(value).map(([key, entryValue]) => [
        key,
        redactStripePaymentMethodIds(entryValue),
      ])
    );
  }
...
function safeStripeErrorForLog(error) {
  if (!error || typeof error !== 'object') {
    return redactStripePaymentMethodIds(String(error || 'unknown_error'));
  }
...
function publicPaymentMethodReferenceForLog(value) {
  const storedReferenceId = parsePublicPaymentMethodReference(value);
...
function assertSucceededDepositPaymentForSettlement(depositPayment, booking, {
  ...
  const allowedStatuses = allowPartiallyRefunded
    ? new Set(['succeeded', 'partially_refunded'])
    : new Set(['succeeded']);
  if (!allowedStatuses.has(depositStatus)) {
    throw createBookingFinancialIntegrityError(
...
function mapStoredPaymentMethodForApi(row = {}) {
  ...
  const recordId = normalizeNullableInteger(row.selected_customer_payment_method_id ?? row.id);

  return {
    id: buildPublicPaymentMethodReference(recordId),
    last4,
...
function assertBookingStatusUpdateAllowed({
  ...
  if (['canceled', 'expired', 'finished'].includes(normalizedCurrentServiceStatus)) {
    throwValidationError('La reserva ya está cerrada y no puede cambiar de estado desde este endpoint.');
  }

  if (['pending_deposit', 'requested'].includes(normalizedNextServiceStatus)) {
    throwValidationError('No se puede retroceder el estado de la reserva desde este endpoint.');
  }
  ...
  switch (normalizedNextServiceStatus) {
    ...
    case 'canceled':
      ...
      if (!['requested', 'accepted', 'in_progress'].includes(normalizedCurrentServiceStatus)) {
        throwValidationError('La reserva no se puede cancelar desde este estado.');
      }
      if (
...
async function ensureRefundForPaymentIntent({
  ...
  const normalizedRefundRequestId = normalizeRefundRequestId(refundRequestId || metadata?.refund_request_id)
    || buildRefundRequestId([
      'ephemeral',
      paymentIntentId,
  ...
  return {
    ...
    refundRequest: shouldRequestDepositRefund
      ? {
          ...
          refundRequestId: buildRefundRequestId([
            reasonCode || 'deposit_refund_requested',
            currentBooking.id,
            depositPayment.id,
          ]),
        }
...
async function getStoredCustomerPaymentMethodByPublicReference(conn, userId, paymentMethodReference, { forUpdate = false } = {}) {
  const normalizedUserId = normalizeNullableInteger(userId);
  const paymentMethodRecordId = parsePublicPaymentMethodReference(paymentMethodReference);

  if (!normalizedUserId || !paymentMethodRecordId) {
    return null;
  }
  ...
  const [rows] = await conn.query(
    ...
    ${forUpdate ? 'FOR UPDATE' : ''}
    `,
    [paymentMethodRecordId, normalizedUserId]
  );

  return rows[0] || null;
}

async function resolveCustomerPaymentMethodInput(conn, userId, paymentMethodInput, { forUpdate = false } = {}) {
  const normalizedInput = typeof paymentMethodInput === 'string' ? paymentMethodInput.trim() : '';
  if (!normalizedInput) {
    return null;
  }
  ...
  if (storedPaymentMethod?.customer_payment_method_stripe_id) {
    return {
      stripePaymentMethodId: storedPaymentMethod.customer_payment_method_stripe_id,
      storedPaymentMethod,
      publicReference: buildPublicPaymentMethodReference(storedPaymentMethod.id),
    };
  }
  ...
  for (const paymentMethodId of paymentMethodIdsToDetach) {
    try {
      ...
      console.error('Error detaching ephemeral Stripe payment method:', {
        bookingId: normalizedBookingId,
        paymentMethodRef: publicPaymentMethodReferenceForLog(paymentMethodId),
...
async function resolvePaymentIntentPersistence(intentLike, { paymentMethodFallback = null } = {}) {
  ...
  return {
    ...
    lastErrorMessage: redactStripePaymentMethodIds(lastError?.message || null),
  };
}
...
async function upsertPaymentRow(conn, { bookingId, type, amountCents, commissionSnapshotCents, finalPriceSnapshotCents, status, currency }) {
  ...
  const [existingRows] = await conn.query(
    `SELECT id, amount_cents, commission_snapshot_cents, final_price_snapshot_cents, status, currency
     FROM payments
     WHERE booking_id = ? AND type = ?
     ORDER BY id DESC
     LIMIT 1
  ...
  if (existingRows.length > 0) {
    ...
    const protectExistingSnapshot = PAYMENT_STATUSES_PROTECTED_FROM_SUCCESS_REGRESSION.has(existingStatus);
    const statusToPersist = protectExistingSnapshot && existingStatus !== incomingStatus
      ? existingStatus
      : incomingStatus;
    ...
    await conn.query(
      `UPDATE payments
       ...
       WHERE id = ?`,
      [
        protectExistingSnapshot ? existingPayment.amount_cents : amountCents,
        protectExistingSnapshot ? existingPayment.commission_snapshot_cents : commissionSnapshotCents,
        protectExistingSnapshot ? existingPayment.final_price_snapshot_cents : finalPriceSnapshotCents,
        statusToPersist,
...
async function upsertPayment(conn, { bookingId, type, paymentIntentId, amountCents, commissionSnapshotCents, finalPriceSnapshotCents, status, currency, transferGroup, paymentMethodId, paymentMethodLast4, lastErrorCode, lastErrorMessage }) {
  ...
  if (normalizedPaymentIntentId) {
    const [intentRows] = await conn.query(
      `SELECT id, payment_intent_id, amount_cents, commission_snapshot_cents, final_price_snapshot_cents,
              status, currency, transfer_group, payment_method_id, payment_method_last4
       FROM payments
       WHERE payment_intent_id = ?
       LIMIT 1
       FOR UPDATE`,
  ...
  const updateParams = [
    ...
    protectExistingSnapshot ? existingPayment.payment_method_id : (paymentMethodId || null),
    protectExistingSnapshot ? existingPayment.payment_method_last4 : (paymentMethodLast4 || null),
    lastErrorCode ?? null,
    lastErrorMessage ?? null,
  ];
...
function normalizeBookingPaymentType(value) {
  const normalizedType = String(value || '').trim().toLowerCase();
  return BOOKING_PAYMENT_TYPES.has(normalizedType) ? normalizedType : null;
}
...
function getStripeChargeRefundStatus(charge) {
  if (!charge) {
    return { refundedAmountCents: 0, capturedAmountCents: 0, fullyRefunded: false };
  }

  const refundedAmountCents = normalizeStripeAmountCents(charge.amount_refunded);
  const capturedAmountCents = normalizeStripeAmountCents(
...
async function validateStripePaymentIntentForBookingWebhook(connection, pi, {
  ...
  const normalizedBookingId = normalizeNullableInteger(bookingId);
  const normalizedType = normalizeBookingPaymentType(type);
  const paymentIntentId = typeof pi?.id === 'string' ? pi.id : null;

  if (!normalizedBookingId || !normalizedType || !paymentIntentId) {
    return {
      valid: false,
      bookingId: normalizedBookingId,
      type: normalizedType,
      reason: 'invalid_payment_intent_metadata',
    };
  }

  const [[paymentByIntent]] = await connection.query(
    `SELECT id, booking_id, type, payment_intent_id, amount_cents, status, currency, transfer_group
     FROM payments
     WHERE payment_intent_id = ?
     LIMIT 1
     FOR UPDATE`,
  ...
  const [[paymentByScope]] = await connection.query(
    `SELECT id, booking_id, type, payment_intent_id, amount_cents, status, currency, transfer_group
     FROM payments
     WHERE booking_id = ? AND type = ?
     LIMIT 1
  ...
  const paymentRow = paymentByIntent || paymentByScope || null;
  if (!paymentRow) {
    return {
      valid: false,
      bookingId: normalizedBookingId,
      type: normalizedType,
      reason: 'payment_row_missing',
    };
  }
  ...
  if (
    paymentByIntent
    && (
      Number(paymentByIntent.booking_id) !== normalizedBookingId
      || normalizeBookingPaymentType(paymentByIntent.type) !== normalizedType
    )
  ) {
    return {
      valid: false,
      ...
      type: normalizedType,
      paymentRow,
      reason: 'payment_intent_scope_mismatch',
    };
  }

  if (paymentByIntent && paymentByScope && Number(paymentByIntent.id) !== Number(paymentByScope.id)) {
    return {
      valid: false,
      bookingId: normalizedBookingId,
      type: normalizedType,
      paymentRow,
      reason: 'payment_intent_conflicts_with_booking_type',
    };
  }

  if (paymentByScope?.payment_intent_id && paymentByScope.payment_intent_id !== paymentIntentId) {
    return {
      valid: false,
      bookingId: normalizedBookingId,
      type: normalizedType,
      paymentRow: paymentByScope,
      reason: 'booking_type_already_has_different_payment_intent',
    };
  }
  ...
  const expectedAmountCents = normalizeStripeAmountCents(paymentRow.amount_cents);
  const intentAmountCents = normalizeStripeAmountCents(pi.amount);
  if (expectedAmountCents > 0 && intentAmountCents !== expectedAmountCents) {
    return {
      valid: false,
      bookingId: normalizedBookingId,
      type: normalizedType,
      ...
      expectedAmountCents,
      actualAmountCents: intentAmountCents,
    };
  }

  const receivedAmountCents = normalizeStripeAmountCents(pi.amount_received);
  if (eventType === 'payment_intent.succeeded' && expectedAmountCents > 0 && receivedAmountCents !== expectedAmountCents) {
    return {
      valid: false,
      bookingId: normalizedBookingId,
      type: normalizedType,
      booking,
      paymentRow,
      reason: 'payment_intent_received_amount_mismatch',
      expectedAmountCents,
      actualAmountCents: receivedAmountCents,
    };
  }
  ...
  if (intentCustomerId !== expectedCustomerId) {
    return {
      ...
      expectedCustomerId,
      actualCustomerId: intentCustomerId,
    };
  }

  const expectedTransferGroup = paymentRow.transfer_group || `booking-${normalizedBookingId}`;
  if (pi.transfer_group && expectedTransferGroup && pi.transfer_group !== expectedTransferGroup) {
    return {
      valid: false,
      bookingId: normalizedBookingId,
      type: normalizedType,
      booking,
      paymentRow,
      reason: 'payment_intent_transfer_group_mismatch',
      expectedTransferGroup,
      actualTransferGroup: pi.transfer_group,
    };
  }
...
async function flagBookingPaymentForManualReview(connection, bookingId, {
  changedByUserId = null,
  nextSettlementStatus = 'manual_review_required',
  reasonCode = 'payment_manual_review_required',
  details = 'La reserva requiere revisión manual de pagos.',
  issueType = 'payment_dispute',
} = {}) {
  const normalizedBookingId = normalizeNullableInteger(bookingId);
  if (!normalizedBookingId) {
...
async function deriveAndSyncBookingRefundSettlementStatus(connection, bookingId, {
  changedByUserId = null,
  reasonCode = 'booking_refund_state_synced',
} = {}) {
  const normalizedBookingId = normalizeNullableInteger(bookingId);
  if (!normalizedBookingId) {
    return null;
  ...
  const [paymentRows] = await connection.query(
    `SELECT id, type, amount_cents, status
     FROM payments
     WHERE booking_id = ? AND type IN ('deposit', 'final')
     FOR UPDATE`,
    [normalizedBookingId]
  );

  const financialRows = (paymentRows || []).filter((row) => {
    const amountCents = normalizeStripeAmountCents(row.amount_cents);
    const status = String(row.status || '').trim().toLowerCase();
    return amountCents > 0 && FINANCIAL_PAYMENT_STATUSES.has(status);
  });

  if (financialRows.length === 0) {
    return normalizeSettlementStatus(booking.settlement_status, 'none');
  }

  const statuses = financialRows.map((row) => String(row.status || '').trim().toLowerCase());
  const hasRefundPending = statuses.includes('refund_pending');
  const hasRefunded = statuses.includes('refunded');
  const hasPartiallyRefunded = statuses.includes('partially_refunded');
  const allFinancialPaymentsRefunded = statuses.every((status) => status === 'refunded');

  let nextSettlementStatus = normalizeSettlementStatus(booking.settlement_status, 'none');
  if (allFinancialPaymentsRefunded) {
    nextSettlementStatus = 'refunded';
  } else if (hasRefundPending) {
    nextSettlementStatus = 'refund_pending';
  } else if (hasRefunded || hasPartiallyRefunded) {
    nextSettlementStatus = 'partially_refunded';
  }

  if (nextSettlementStatus !== normalizeSettlementStatus(booking.settlement_status, 'none')) {
    await transitionBookingStateRecord(connection, booking, {
      nextServiceStatus: booking.service_status,
      nextSettlementStatus,
      changedByUserId,
  ...
  return nextSettlementStatus;
}

async function syncPaymentRefundStateFromStripe(paymentIntentId, {
  reasonCode = 'stripe_refund_synced',
} = {}) {
  const normalizedPaymentIntentId = typeof paymentIntentId === 'string' ? paymentIntentId.trim() : '';
  if (!normalizedPaymentIntentId) {
  ...
  const intent = await retrievePaymentIntentWithCharge(normalizedPaymentIntentId);
  const latestCharge = intent?.latest_charge;
  if (!latestCharge) {
    return null;
  }

  const { refundedAmountCents, fullyRefunded } = getStripeChargeRefundStatus(latestCharge);
  ...
  try {
    await connection.beginTransaction();
    const [[payment]] = await connection.query(
      'SELECT id, booking_id, type FROM payments WHERE payment_intent_id = ? FOR UPDATE',
      [normalizedPaymentIntentId]
    );
    ...
    await connection.query(
      'UPDATE payments SET status = ? WHERE id = ?',
      [fullyRefunded ? 'refunded' : 'partially_refunded', payment.id]
    );

    const nextSettlementStatus = await deriveAndSyncBookingRefundSettlementStatus(connection, payment.booking_id, {
      reasonCode,
    });
...
async function verifyPaymentIntentCapturedForSettlement({
  paymentIntentId,
  expectedAmountCents,
  expectedCurrency,
  expectedCustomerId = null,
  allowRefunded = false,
  minimumNetCapturedAmountCents = null,
} = {}) {
  const normalizedPaymentIntentId = typeof paymentIntentId === 'string' ? paymentIntentId.trim() : '';
  const normalizedExpectedAmount = normalizeStripeAmountCents(expectedAmountCents);
  const normalizedExpectedCurrency = normalizeCurrencyCode(expectedCurrency, null);
  if (!normalizedPaymentIntentId || normalizedExpectedAmount <= 0) {
    return { valid: false, reason: 'invalid_payment_intent_verification_input' };
  }

  const intent = await retrievePaymentIntentWithCharge(normalizedPaymentIntentId);
  const latestCharge = intent?.latest_charge;
  if (!intent || !latestCharge) {
    return { valid: false, reason: 'payment_intent_charge_missing' };
  }

  if (intent.status !== 'succeeded') {
    return { valid: false, reason: 'payment_intent_not_succeeded', status: intent.status };
  }

  if (latestCharge.captured === false || normalizeStripeAmountCents(intent.amount_capturable) > 0) {
    return {
      valid: false,
      reason: 'payment_intent_not_fully_captured',
      amountCapturableCents: normalizeStripeAmountCents(intent.amount_capturable),
    };
  }
  ...
  if (expectedCustomer && expectedCustomer !== actualCustomer) {
    return {
      ...
      reason: 'payment_intent_customer_mismatch',
      expectedCustomerId: expectedCustomer,
      actualCustomerId: actualCustomer,
    };
  }

  const intentAmountCents = normalizeStripeAmountCents(intent.amount);
  if (intentAmountCents !== normalizedExpectedAmount) {
    return {
      valid: false,
      reason: 'payment_intent_amount_mismatch',
      expectedAmountCents: normalizedExpectedAmount,
      actualAmountCents: intentAmountCents,
    };
  }

  const { refundedAmountCents } = getStripeChargeRefundStatus(latestCharge);
  if (refundedAmountCents > 0 && !allowRefunded) {
    return {
      valid: false,
      reason: 'payment_intent_already_refunded',
      refundedAmountCents,
    };
  }

  const amountReceivedCents = normalizeStripeAmountCents(intent.amount_received);
  const capturedAmountCents = normalizeStripeAmountCents(
    latestCharge.amount_captured ?? latestCharge.amount
  );
  if (
    (amountReceivedCents > 0 && amountReceivedCents !== normalizedExpectedAmount)
    || capturedAmountCents !== normalizedExpectedAmount
  ) {
    return {
      valid: false,
      reason: 'payment_intent_captured_amount_mismatch',
      expectedAmountCents: normalizedExpectedAmount,
      actualAmountCents: Math.max(amountReceivedCents, capturedAmountCents),
    };
  }

  const netCapturedAmountCents = Math.max(0, capturedAmountCents - refundedAmountCents);
  const normalizedMinimumNetCapturedAmount = minimumNetCapturedAmountCents === null || minimumNetCapturedAmountCents === undefined
    ? null
    : normalizeStripeAmountCents(minimumNetCapturedAmountCents);
  if (
    normalizedMinimumNetCapturedAmount !== null
    && netCapturedAmountCents < normalizedMinimumNetCapturedAmount
  ) {
    return {
      valid: false,
      reason: 'payment_intent_net_captured_amount_too_low',
      expectedNetCapturedAmountCents: normalizedMinimumNetCapturedAmount,
      actualNetCapturedAmountCents: netCapturedAmountCents,
      refundedAmountCents,
    };
  }

  return { valid: true, intent, latestCharge, refundedAmountCents, capturedAmountCents, netCapturedAmountCents };
}
...
async function markBookingSettlementForReview(bookingId, {
  ...
  try {
    await connection.beginTransaction();
    const flagged = await flagBookingPaymentForManualReview(connection, normalizedBookingId, {
      changedByUserId,
      nextSettlementStatus,
      reasonCode,
      details,
...
async function executeRefundRequestOrFlagForManualReview(refundRequest, metadata = {}, {
  ...
  const refundRequestId = normalizeRefundRequestId(refundRequest.refundRequestId || metadata?.refund_request_id)
    || buildRefundRequestId([
      metadata?.source || refundRequest.reasonCode || 'booking_refund',
      refundRequest.bookingId || '',
      metadata?.payment_id || refundRequest.paymentId || '',
  ...
  try {
    await triggerStripeRefundForPaymentIntent(refundRequest.paymentIntentId, refundMetadata, {
      refundRequestId,
    });
    return true;
  } catch (refundError) {
    ...
    await markRefundFailureForManualReview({
      ...
      source: refundMetadata?.source || refundRequest.reasonCode || 'booking_refund',
      error: refundError,
    });
    return false;
  }
...
async function applySuccessfulDepositPaymentForBooking(connection, {
  ...
  if (['canceled', 'expired'].includes(normalizedServiceStatus) && !bookingRow.deposit_confirmed_at) {
    ...
    return {
      ...
      refundRequest: {
        bookingId: normalizedBookingId,
        paymentId: depositPaymentRow?.id || null,
        paymentIntentId: intent.id,
        reasonCode: staleReasonCode,
        refundRequestId: buildRefundRequestId([
          staleReasonCode,
          normalizedBookingId,
          depositPaymentRow?.id || intent.id,
        ]),
      },
    };
  }
...
async function finalizeBookingSettlementAfterSuccessfulPayment(bookingId, {
  ...
  try {
    ...
    if (
      ...
      && (
        !finalPayment?.payment_intent_id
        || finalPaymentStatus !== 'succeeded'
        || normalizeStripeAmountCents(finalPayment.amount_cents) < settlementSnapshot.amountDueFromClientCents
      )
    ) {
      await initialConnection.rollback();
  ...
  try {
    ...
    if (settlementSnapshot.amountDueFromClientCents > 0 && finalPayment?.payment_intent_id) {
      const paymentVerification = await verifyPaymentIntentCapturedForSettlement({
        ...
        expectedAmountCents: settlementSnapshot.amountDueFromClientCents,
        expectedCurrency: settlementSnapshot.chargeCurrency,
        expectedCustomerId: booking.customer_id,
      });
      ...
      if (!paymentVerification.valid) {
        await markBookingSettlementForReview(normalizedBookingId, {
          changedByUserId,
          nextSettlementStatus: 'manual_review_required',
          reasonCode: 'final_payment_integrity_failed',
          details: `El pago final no supera la verificación de Stripe: ${paymentVerification.reason}.`,
        });
        return { settled: false, reason: paymentVerification.reason };
      }
    }
  ...
  try {
    if (settlementSnapshot.amountToRefundCents > 0 && depositPayment?.payment_intent_id) {
      await ensureRefundForPaymentIntent({
        ...
        refundRequestId: buildRefundRequestId([
          'booking_settlement_refund',
          normalizedBookingId,
          booking.closure_proposal_id || '',
          depositPayment.id || '',
          settlementSnapshot.amountToRefundCents,
        ]),
        transferGroup,
        metadata: {
  ...
  try {
    ...
    try {
      assertSucceededDepositPaymentForSettlement(currentDepositPayment, currentBooking, {
        requirePaymentIntent: true,
        allowPartiallyRefunded: settlementSnapshot.amountToRefundCents > 0,
      });
      if (settlementSnapshot.amountDueFromClientCents > 0) {
...
async function processPendingClosureAutoCharge(bookingId) {
  ...
  try {
    ...
    const depositStripeVerification = await verifyPaymentIntentCapturedForSettlement({
      paymentIntentId: depositPayment.payment_intent_id,
      expectedAmountCents: depositPayment.amount_cents,
      expectedCurrency: resolveRequiredBookingServiceCurrency(booking),
      expectedCustomerId: booking.customer_id,
    });
    if (!depositStripeVerification.valid) {
      await connection.rollback();
      await markBookingSettlementForReview(normalizedBookingId, {
        nextSettlementStatus: 'in_dispute',
        reasonCode: 'auto_charge_deposit_payment_integrity_failed',
        details: `El depósito no supera la verificación de Stripe antes del auto-cobro: ${depositStripeVerification.reason}.`,
      });
      return {
        handled: false,
        reason: depositStripeVerification.reason,
      };
    }
  ...
  try {
    ...
    console.error('Error processing closure auto-charge:', {
      bookingId: normalizedBookingId,
      error: error?.type && String(error.type).startsWith('Stripe')
        ? safeStripeErrorForLog(error)
        : redactStripePaymentMethodIds(error.message),
    });
    const failureDetails = 'El cobro automático ha fallado con el método de pago guardado.';
    if (!isSecondaryReminderWindow) {
      try {
        await scheduleClosureFollowUpWindow({
          details: failureDetails,
...
async function releasePendingProviderPayouts({
  ...
  const normalizedNow = parseDateTimeInput(now) || new Date();
  if (!isFirstWednesdayMorningPayoutWindow(normalizedNow)) {
    return {
      releasedPayments: 0,
      releasedAmountCents: 0,
      skippedPayments: 0,
      deferredUntilNextPayoutWindow: true,
    };
  }
  ...
  try {
    ...
    const [rows] = await connection.query(
      ...
      INNER JOIN payments deposit ON deposit.booking_id = p.booking_id
        ...
        AND deposit.status IN ('succeeded', 'partially_refunded')
    ...
    for (const [, group] of groupedRows) {
      ...
      for (const payment of group.payments) {
        ...
        try {
          const depositVerification = await verifyPaymentIntentCapturedForSettlement({
            paymentIntentId: payment.deposit_payment_intent_id,
            expectedAmountCents: payment.deposit_amount_cents,
            expectedCurrency: payment.deposit_payment_currency || group.currency,
            allowRefunded: true,
          });
          if (!depositVerification.valid) {
            await flagBookingPaymentForManualReview(connection, payment.booking_id, {
              nextSettlementStatus: 'manual_review_required',
              reasonCode: 'provider_payout_deposit_integrity_failed',
              details: `El depósito no supera la verificación de Stripe antes del payout: ${depositVerification.reason}.`,
            });
            skippedPayments += 1;
            continue;
          }

          if (normalizeStripeAmountCents(payment.amount_cents) > 0) {
            const finalPaymentVerification = await verifyPaymentIntentCapturedForSettlement({
              paymentIntentId: payment.payment_intent_id,
              expectedAmountCents: payment.amount_cents,
              expectedCurrency: group.currency,
            });
            if (!finalPaymentVerification.valid) {
              await flagBookingPaymentForManualReview(connection, payment.booking_id, {
                nextSettlementStatus: 'manual_review_required',
                reasonCode: 'provider_payout_final_payment_integrity_failed',
                details: `El pago final no supera la verificación de Stripe antes del payout: ${finalPaymentVerification.reason}.`,
              });
              skippedPayments += 1;
              continue;
            }
...
// cron semanal; la función solo libera fondos si es el primer miércoles por la mañana en Europe/Madrid
...
app.post('/api/bookings', authenticateToken, bookingCreateLimiter, async (req, res) => {
  ...
  try {
    ...
    if (Number(requestedUserId) === Number(serviceSnapshot.provider_user_id_snapshot)) {
      await connection.rollback();
      return res.status(409).json({
...
app.patch('/api/bookings/:id/update-data', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(req.body || {}, 'address_id') && !isStaff) {
      await connection.rollback();
      return res.status(409).json({
        error: 'La dirección solo puede modificarse mediante la solicitud formal de modificación.',
        error_code: 'booking_address_change_request_required',
      });
    }
...
app.post('/api/bookings/:id/dispute/cancel', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (normalizedIssueType === 'payment_dispute') {
      ...
      const restoredClientApprovalDeadline = shouldRestorePendingApproval
        ? parseDateTimeInput(latestProposal.auto_charge_scheduled_at)
        : null;
      ...
      await transitionBookingStateRecord(connection, booking, {
        ...
        extraPatch: {
          client_approval_deadline_at: restoredClientApprovalDeadline,
        },
      });
    } else if (
...
app.post('/api/bookings/:id/payment-sync', authenticateToken, bookingPaymentLimiter, async (req, res) => {
  ...
  try {
    ...
    console.error('Error resolviendo PaymentIntent para sincronización:', {
      ...
      paymentIntentId,
      type,
      error: redactStripePaymentMethodIds(persistenceError.message),
    });
  ...
  try {
    ...
    const [rows] = await connection.query(
      ...
      SELECT
             ...
             cp.proposed_total_amount_cents,
             s.id AS service_id,
             s.user_id AS effective_provider_user_id,
    ...
    if (Number(booking.user_id) === Number(booking.provider_user_id_snapshot || booking.effective_provider_user_id)) {
      await connection.rollback();
      return res.status(409).json({
        error: 'No puedes reservar tu propio servicio.',
        error_code: 'self_booking_not_allowed',
...
const round2 = (n) => {
  ...
  return Math.round((x + Number.EPSILON) * 100) / 100;
    ...
    // Congelar importes solo si aún no existe fila. Los reintentos deben reutilizar
    // el PaymentIntent/fila existente y no degradar un depósito cobrado.
    if (existingDepositPayment) {
      payment = existingDepositPayment;
      const existingDepositStatus = String(payment.status || '').trim().toLowerCase();
      if (
        PAYMENT_STATUSES_PROTECTED_FROM_SUCCESS_REGRESSION.has(existingDepositStatus)
        && existingDepositStatus !== 'succeeded'
      ) {
        await connection.rollback();
        return res.status(409).json({
          error: 'El depósito ya está en un estado financiero protegido.',
          error_code: 'deposit_payment_state_locked',
          payment_status: existingDepositStatus,
        });
      }
      if (existingDepositStatus === 'succeeded' && !payment.payment_intent_id) {
        throw createBookingFinancialIntegrityError(
          'deposit_payment_intent_missing',
          'El depósito confirmado no tiene PaymentIntent asociado.',
          412
    ...
    try {
      ...
      if (['requires_payment_method', 'requires_confirmation'].includes(intent.status) && resolvedPaymentMethodId) {
        ...
        if (!pm.customer) {
          try {
            pm = await stripe.paymentMethods.attach(resolvedPaymentMethodId, { customer: customerId });
          } catch (eAttach) {
            console.error('No se pudo adjuntar el PM al customer (depósito):', safeStripeErrorForLog(eAttach));
      ...
      console.error('Error de Stripe al crear/recuperar PaymentIntent:', {
        bookingId: id,
        userId: req.user?.id,
        stripeError: safeStripeErrorForLog(stripeError),
        paymentData: {
          amountCents: commissionCents,
          customerId,
          transferGroup,
      ...
      if (pi && pi.id) {
        ...
        try {
          ...
          await upsertPayment(connErr, {
            ...
            currency: chargeCurrency,
            transferGroup,
            paymentMethodId: paymentPersistence.paymentMethodId || resolvedPaymentMethodId || null,
            paymentMethodLast4: paymentPersistence.paymentMethodLast4 || null,
            lastErrorCode: paymentPersistence.lastErrorCode,
            lastErrorMessage: paymentPersistence.lastErrorMessage,
          });
    ...
    console.error('Error en depósito de reserva:', {
      bookingId: id,
      userId: req.user?.id,
      error: err?.type && String(err.type).startsWith('Stripe')
        ? safeStripeErrorForLog(err)
        : redactStripePaymentMethodIds(err.message),
    });
    ...
    if (err.type && err.type.startsWith('Stripe')) {
      return res.status(400).json({
        error: 'No se pudo procesar el depósito con el método de pago indicado.',
        error_code: err.code || 'stripe_deposit_failed',
      });
    }

    return res.status(400).json({
      error: 'No se pudo crear el depósito.',
      details: redactStripePaymentMethodIds(err.message)
    });
  } finally {
...
app.post('/api/bookings/:id/final-payment-transfer', authenticateToken, bookingPaymentLimiter, async (req, res) => {
  ...
  try {
    ...
    const usablePaymentMethodId = resolvedPaymentMethodId || await getBookingSelectedStripePaymentMethodId(connection, id, { forUpdate: true });
    ...
    if (!depositStripeVerification.valid) {
      await flagBookingPaymentForManualReview(connection, id, {
        changedByUserId: req.user?.id || null,
        nextSettlementStatus: 'manual_review_required',
        reasonCode: 'deposit_payment_integrity_failed',
        details: `El depósito no supera la verificación de Stripe antes del cobro final: ${depositStripeVerification.reason}.`,
      });
      await connection.commit();
      return res.status(409).json({
        error: 'El depósito requiere revisión manual antes del cobro final.',
        error_code: 'deposit_payment_integrity_failed',
        reason: depositStripeVerification.reason,
      });
    }
    ...
    if (existingFinal.length > 0) {
      ...
      try {
          ...
          try {
            ...
            if (!pm2.customer) {
              try {
                ...
                console.error('No se pudo adjuntar el PM al customer:', safeStripeErrorForLog(e1));
                return res.status(400).json({ error: 'No se pudo adjuntar el método de pago al cliente.' });
              }
            }
            ...
            console.error('Error al adjuntar/confirmar PM en intent existente:', safeStripeErrorForLog(e1));
        ...
        console.error('Error recuperando PaymentIntent existente:', safeStripeErrorForLog(e));
    ...
    if (!pm.customer) {
      try {
        ...
        console.error('No se pudo adjuntar el PM al customer:', safeStripeErrorForLog(e));
        return res.status(400).json({ error: 'No se pudo adjuntar el método de pago al cliente.' });
      }
    }
    ...
    try {
      ...
      if (pi && pi.id) {
        ...
        if (pi.status === 'requires_payment_method') {
          const reaction = await scheduleFinalPaymentFailureReactionWindow(id, {
            changedByUserId: req.user?.id || null,
            details: paymentPersistence.lastErrorMessage || 'El cobro final requiere un nuevo método de pago.',
            followUpReasonCode: 'final_payment_failed_follow_up',
            disputeReasonCode: 'final_payment_failed_dispute',
          });
      ...
      const reaction = await scheduleFinalPaymentFailureReactionWindow(id, {
        changedByUserId: req.user?.id || null,
        details: 'No se pudo procesar el pago final con el método de pago indicado.',
        followUpReasonCode: 'final_payment_failed_follow_up',
        disputeReasonCode: 'final_payment_failed_dispute',
      });
      await sendFinalPaymentReactionEmailIfNeeded(id, reaction, 'final payment Stripe failure');
...
app.post('/api/services/:id/reviews', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (reviewTrackingSupported && !normalizedBookingId) {
      return res.status(400).json({
        error: 'booking_id es requerido para valorar un servicio.',
        error_code: 'booking_id_required_for_review',
      });
    }

    if (reviewTrackingSupported) {
      const [bookingRows] = await promisePool.query(
        ...
        FROM booking
        LEFT JOIN service s ON s.id = booking.service_id
        WHERE id = ?
        LIMIT 1
        `,
        [normalizedBookingId]
      ...
      if (Number(booking.client_user_id) === Number(booking.provider_user_id_snapshot || booking.effective_provider_user_id)) {
        return res.status(409).json({
          error: 'No puedes valorar una reserva de tu propio servicio.',
          error_code: 'self_booking_review_not_allowed',
        });
      }
  ...
  try {
    switch (event.type) {
      ...
      case 'payment_intent.canceled': {
        ...
        const bookingId = normalizeNullableInteger(pi?.metadata?.booking_id);
        const type = normalizeBookingPaymentType(pi?.metadata?.type);
        ...
        await connection.beginTransaction();
        const webhookValidation = await validateStripePaymentIntentForBookingWebhook(connection, pi, {
          bookingId,
          type,
        ...
        if (!webhookValidation.valid) {
          const details = `Webhook de Stripe rechazado para PaymentIntent ${pi?.id || 'desconocido'} (${event.type}): ${webhookValidation.reason}.`;
          await flagBookingPaymentForManualReview(connection, webhookValidation.bookingId || bookingId, {
            nextSettlementStatus: 'manual_review_required',
            reasonCode: 'stripe_webhook_validation_failed',
            details,
          });
          await connection.commit();
          console.error('Stripe webhook payment validation failed:', {
            eventType: event.type,
            paymentIntentId: pi?.id || null,
            bookingId,
            type,
            reason: webhookValidation.reason,
            expectedAmountCents: webhookValidation.expectedAmountCents,
            actualAmountCents: webhookValidation.actualAmountCents,
            expectedCurrency: webhookValidation.expectedCurrency,
            actualCurrency: webhookValidation.actualCurrency,
          });
          break;
        }

        const amountCents = webhookValidation.amountCents;
        await upsertPayment(connection, {
          bookingId,
          type,
          paymentIntentId: pi.id,
      ...
      case 'charge.refunded': {
        ...
        if (payment) {
          refundedBookingId = payment.booking_id;
          await connection.query(
            'UPDATE payments SET status = ? WHERE id = ?',
            [fullyRefunded ? 'refunded' : 'partially_refunded', payment.id]
          );
